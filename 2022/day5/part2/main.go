...
func executeMove(in *instruction, stacks map[int][]string) map[int][]string {
	...
	fmt.Printf("sourceStack before = %+v\n", sourceStack)
	// get the last value (top) of the stack to move
	sourceValues := sourceStack[len(sourceStack)-in.count:]
	fmt.Printf("sourceValues = %+v\n", sourceValues)

	sourceStack = sourceStack[:len(sourceStack)-in.count]
	fmt.Printf("sourceStack after = %+v\n", sourceStack)
	destStack := stacks[in.dest]
	// move the sourceValues to the top of the destStack
	destStack = append(destStack, sourceValues...)
